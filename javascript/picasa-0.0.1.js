	...
	$j(document).ready(function(){
		$j.getJSON(
			...
			function(data, status) {
				...
				$j("#" + idDiv).prepend("<div class='col-xs-12' id=\"picasaThumb_" + albumid + "\"></div>");									
				...
				$j.each(data.feed.entry, function(i, pic) {
					...
					if (tmpCount) {
						$j("<img/>").attr("src", thumb.url.replace("s288","s300"))
	...
	$j(document).ready(function(){
		$j.getJSON(
			...
			function(data, status) {
				...
				$j("#" + albumid).append("<div class='col-xs-12'><div class='row' id='albumInfor'></div><hr class='hrLine'/>​");
